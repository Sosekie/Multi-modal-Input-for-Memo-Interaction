...
import threading
import queue
from utils.function_new import merge, create, open, add_close, write
import cv2
...
def function_wrapper(func, *args):
    ...
    with function_lock:
        if len(running_functions) >= 1:
            return
        running_functions.append(func.__name__)
    try:
        func(*args)
    finally:
        with function_lock:
            running_functions.remove(func.__name__)
            ...
            if ret:
                ...
                if detection_result:
                    ...
                    if len(triggered_memo_list) >= 2:
                        threading.Thread(target=function_wrapper, args=(merge, triggered_memo_list[0], triggered_memo_list[1], memo_list, opened_memo, audio_done_event_merge, last_audio_trigger_time, audio_trigger_interval, result_queue_merge)).start()
                            ...
                            if pinched_memo is not None:
                                ...
                                if pinched_memo == opened_memo:
                                    if pinched_memo.is_added:
                                        # write
                                        threading.Thread(target=function_wrapper, args=(write, pinched_memo, audio_pipe, last_audio_trigger_time, audio_trigger_interval, result_queue_write, audio_done_event_write)).start()
                                    else:
                                        # add
                                        threading.Thread(target=function_wrapper, args=(add_close, opened_memo, pinched_memo, audio_done_event_add, last_audio_trigger_time, audio_trigger_interval, result_queue_add)).start()
                                else:
                                    # open
                                    threading.Thread(target=function_wrapper, args=(open, opened_memo, pinched_memo, audio_done_event_open, last_audio_trigger_time, audio_trigger_interval, result_queue_open)).start()
                                ...
                                # create a new memo
                                    ...
                                    threading.Thread(target=function_wrapper, args=(create, pinch_position, last_audio_trigger_time, audio_trigger_interval, result_queue_create, audio_done_event_create)).start()
